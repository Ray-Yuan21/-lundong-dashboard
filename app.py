...
def show_home_page(dashboard):
    ...
    **策略说明与风险提示**
    ...
    - 📅 **调仓周期**：本策略基于 5 个交易日为周期进行动态调整
    - 📊 **历史数据**：所有展示的收益率及回测数据均为历史表现，不代表未来收益
    - ⚠️ **投资建议**：本系统仅供研究学习使用，不构成任何投资建议
    - 💼 **风险提示**：投资有风险，入市需谨慎。请根据自身风险承受能力独立决策
    - 📖 **数据来源**：信号数据基于量化模型生成，仅供参考
    
    *本系统不对使用本数据进行投资决策产生的任何损失承担责任*
    """)
...
def show_analysis_page(dashboard):
    ...
    with col1:
        ...
        **📊 专业版功能（需联系作者获取）**
        ...
        以下高级功能可为您的投资决策提供更深入的支持：
        
        - 📈 **价格曲线叠加分析** - 在K线图上直观显示买卖点位置
        - 🎯 **实时交易信号提醒** - 微信/邮件推送最新信号
        - 📊 **多维度因子分析** - 查看策略使用的因子及其有效性
        - 🔍 **详细持仓分析** - 每日持仓跟踪与归因分析
...
def show_trade_signals(dashboard):
    ...
    if selected_symbol:
        ...
        **📈 完整版功能预告**
        ...
        当前在线版本仅显示买卖信号表格。专业版提供：
        
        - 📊 **价格K线图** - 直观查看历史价格走势
        - 🎯 **买卖点标注** - 在K线图上精确标注每个交易点
        - 📈 **持仓收益可视化** - 实时跟踪每笔交易的盈亏
        - 💰 **收益归因分析** - 了解收益来源
        
        💼 如需获取完整功能，请联系作者（GitHub: github.com/Ray-Yuan21）
        """)
        return
